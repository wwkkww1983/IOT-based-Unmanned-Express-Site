...
__IO uint32_t LocalTime = 0;			///初始化系统此变量用于创建时间引用，增量为10毫秒
...
int main(void)
{
		...
		struct tcp_pcb *pcb;
  	System_Setup();			//初始化系统    
    LwIP_Init();			//初始化LwIP  satck ip地址设置，mac设置
    ...
    /*接收OpenMV传来的数据并发送给数据库*/
		...
		if(flag_uart2_rev==1)
		{
			...
			delay_ms(1000);
			if(p==1)USART1_printf("1Z");			//用户验证码和数据库进行匹配，有则为1，反之为0
