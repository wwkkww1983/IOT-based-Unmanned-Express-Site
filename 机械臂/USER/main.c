...
/**************************数据插补**************************/
/*插补时间间隔为20/12ms，由timer0控制，使舵机平滑实现速度控制；另一个功能是执行完一行后去更新下一行数据，即调用change()*/
void vpwm(void)		 
{
	unsigned char j = 0;
	unsigned char how = 0;
  static unsigned char flag_how;
	...
	else			//不是一行数据全部完成，处于中间插补阶段
	{
		...
		{
			if (abs(CPWM[j] - pos[point_aim][j]) < 5)			//检测靠近终点位置
			{			
			   how++;			//是，则累加一个
		...
		how = 0; 
		if((flag_Tover == 1)&&(flag_how == 1))			//从插补次数，和脉宽宽度两方面都到达终点，本作业行完成
		{								
			 flag_Tover = 0;
			 flag_how = 0;
			 flag_stop = 1;
